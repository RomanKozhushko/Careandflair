...
export function AdminSectionCard({ section, children }: AdminSectionCardProps) {
  return (
    <section id={section.id} className="scroll-mt-6 rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
      <div className="flex flex-col gap-4 border-b border-slate-200 pb-5 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.22em] text-[#9b7b35]">{section.fileName}</p>
          <h2 className="mt-2 text-2xl font-bold tracking-tight text-slate-950">{section.label}</h2>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <button type="button" disabled className="rounded-full border border-slate-200 bg-slate-100 px-4 py-2 text-xs font-bold uppercase tracking-[0.14em] text-slate-400">
            Edit disabled
          </button>
        </div>
      </div>
