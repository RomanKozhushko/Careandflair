...
export function AdminDashboard({ sections, stats, activeRoutes }: AdminDashboardProps) {
  ...
  return (
    <section id="dashboard" className="scroll-mt-6 rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.28em] text-[#9b7b35]">Phase 3</p>
          <h1 className="mt-2 text-3xl font-bold tracking-tight text-slate-950 sm:text-4xl">Admin MVP Foundation</h1>
          <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-600">Preview local JSON content and admin information architecture before Phase 3.1 editable admin work.</p>
        </div>
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-2">
        <div className="rounded-2xl border border-slate-200 p-4">
          <h2 className="text-sm font-bold uppercase tracking-[0.16em] text-slate-500">Active routes</h2>
          <div className="mt-3 flex flex-wrap gap-2">
            {activeRoutes.map((route) => <AdminStatusBadge key={route} status="active">{route}</AdminStatusBadge>)}
          </div>
        </div>
        <div className="rounded-2xl border border-slate-200 p-4">
          <h2 className="text-sm font-bold uppercase tracking-[0.16em] text-slate-500">Section data sources</h2>
          <ul className="mt-3 grid gap-2 text-sm text-slate-700">
            {sections.map((section) => <li key={section.id} className="flex justify-between gap-3"><span>{section.label}</span><span className="font-mono text-xs text-slate-500">{countGroups(section.items)}</span></li>)}
          </ul>
        </div>
      </div>
    </section>
