...
export function BeforeAfterPreview({ content }: { content?: ContentBundle }) {
  ...
  return (
    <section id="before-after" className="relative overflow-hidden bg-[var(--cf-deep)] px-4 py-20 text-white sm:px-6 lg:px-8">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_10%_12%,rgba(118,231,178,0.2),transparent_24%),radial-gradient(circle_at_82%_18%,rgba(184,255,61,0.2),transparent_22%)]" />
      <div className="mx-auto max-w-7xl">
        <div className="relative grid gap-6 lg:grid-cols-[0.82fr_1.18fr] lg:items-end">
          <SectionHeader eyebrow="Before someone books a viewing" title="Proof you can see." subtitle="Most properties do not need a full renovation. They need the visible problems fixed first." light />
          <div className="rounded-3xl border border-white/12 bg-white/10 px-5 py-4 text-sm font-semibold leading-6 text-[var(--cf-mint)] backdrop-blur">
            {section.subtitle}
          </div>
        </div>

        <div className="relative mt-8 grid gap-8 overflow-hidden rounded-[2rem] border border-white/12 bg-white/10 p-4 shadow-[var(--cf-shadow-deep)] backdrop-blur sm:p-6 lg:grid-cols-[1.12fr_0.88fr] lg:items-center">
          <BeforeAfterSlider item={strongestItem} />
          <div className="p-2 sm:p-4">
            <p className="brand-label text-xs brass-text">Common property problem</p>
            <h3 className="mt-3 text-3xl font-semibold tracking-tight text-white">{strongestItem.title}</h3>
            <p className="mt-2 text-sm font-semibold uppercase tracking-[0.16em] text-[var(--cf-mint)]">{strongestItem.category} · {strongestItem.location}</p>
            ...
            <div className="mt-5 grid gap-3 text-sm leading-6 text-[var(--cf-mint)]">
              <p><span className="font-semibold text-white">Problem:</span> {strongestItem.problem}</p>
              <p><span className="font-semibold text-white">Solution:</span> {strongestItem.solution}</p>
