...
export async function getPublicContentBundle(): Promise<ContentBundle> {
  const [
    ...
    readPublicValue<HomepageTransformationsContent>("homepage-transformations", fallbackContent.homepageTransformations),
    readPublicValue<InteractiveToolsConfig>("interactive-tools", fallbackContent.interactiveTools),
    readPublicValue<AudienceMode[]>("audience-modes", fallbackContent.audienceModes),
    readPublicValue<ProblemCategory[]>("problem-categories", fallbackContent.problemCategories),
    readPublicValue<ReadinessScoresConfig>("readiness-scores", fallbackContent.readinessScores),
    ...
    readPublicValue<BeforeAfterMatch[]>("before-after-matches", fallbackContent.beforeAfterMatches),
    readPublicValue<Area[]>("areas", fallbackContent.areas),
    readPublicValue<FaqItem[]>("faqs", fallbackContent.faqs),
    readPublicValue<Visibility[]>("testimonials", fallbackContent.testimonials),
  ]);
